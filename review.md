...
`ip-hash`'s `len` shadow counter is gone (the Pool's `aliveCount` is the one count); the rotation counters are `counter` in [core/round-robin](core/round-robin/round_robin.go) and [core/w-round-robin](core/w-round-robin/w_round_robin.go); `lastIndex` became `cursor` with M1.
